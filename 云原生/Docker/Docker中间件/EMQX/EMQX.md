根据您提供的教程大纲，我为您在原笔记中新增了 **“2.3 MQTT协议报文格式与交互流程详解”** 章节，并补充了关于传输层与应用层关系的感性描述。所有原有内容完整保留，新增部分可无缝插入原有“核心概念理解”小节之后。以下是补充后的完整笔记（新增内容以 **【新增】** 标注，方便定位）。
...
**【新增】感性认识：传输层与应用层的关系**  
MQTT 是应用层协议，依赖 TCP（传输层）提供可靠的双向字节流。TCP 保证数据不丢失、顺序到达，MQTT 在此基础上定义“主题”“服务质量”“遗嘱”等语义。可以把 TCP 看作“电话线路”，MQTT 就是通话时的“语言规则”——线路通了，双方才能按规则对话。
...
**各等级报文详解**：
- **等级0 PUBLISH**（教程大纲第13点）：无可变报头中的 Packet ID，服务端直接转发，不存储离线消息。
- **等级1 PUBLISH + PUBACK**（教程大纲第15点）：发布者保存 Packet ID，等待 PUBACK，超时重发；服务端收到后存储并转发给订阅者，返回 PUBACK。
- **等级2 PUBLISH / PUBREC / PUBREL / PUBCOMP**（教程大纲第16点）：
  1. 发布者发送 PUBLISH（QoS=2, Packet ID = N）。
  2. 服务端回复 PUBREC（N），表示已接收。
  3. 发布者收到 PUBREC 后回复 PUBREL（N），表示可以分发。
  4. 服务端分发完成后回复 PUBCOMP（N），整个流程结束。

#### 2.3.7 保留消息功能详解（教程大纲第14点）

当 PUBLISH 报文设置 Retain = true 时，服务端会为该主题保留最后一条消息（每个主题仅保留最新一条）。新订阅者订阅该主题时，服务端立即推送保留消息（可设置保留消息是否过期）。EMQX 支持通过 Dashboard 或 API 清除保留消息。
