...
#### 1.1.1 HTTP 请求报文（POST 方法，提交 JSON）
...
- **200**（**请求成功**）：GET、PUT、PATCH、DELETE 等操作成功，一切正常，没有问题
- **201**（**资源创建成功**）：POST 请求成功创建了新资源
- **204**（**删除成功，无返回体**）：DELETE 等操作成功，但响应中不包含任何内容

#### 1.3.2 ==**4xx 客户端错误**==

- **400**（**请求参数有误**）：客户端传的参数格式不对、缺少必要字段、或数据校验不通过
- **401**（**未认证**）：请求缺少有效的身份认证信息，或认证已过期
- **403**（**无权限**）：身份已认证，但当前用户没有访问该资源的权限
...
> [!NOTE]
> `401` 和 `403`的区别：`401` 是"你是谁还没搞清楚"（未登录/Token 无效），`403` 是"我知道你是谁，但你不准进"（权限不足）

#### 1.3.3 ==**5xx 服务端错误**==

- **500**（**服务器内部出错**）：服务端代码异常、数据库连接失败、依赖服务不可用等，是后端需要排查的。
- **502**（**错误网关**）：网关/代理服务器从上游服务收到无效响应或超时（如上游进程崩溃、端口不通、响应超时、网关配置不当），通常由运维排查网关配置和网络连通性，后端协助确认服务进程状态。
- **503**（**服务不可用**）：服务器暂时无法处理请求，通常因过载（流量突增）、主动限流、维护中或下游服务熔断降级所致，也可能因资源耗尽（连接池、线程池、内存）而拒绝新请求。排查需区分是容量不足需扩容、限流策略触发（如令牌桶）、还是依赖故障导致级联不可用，由后端和运维协同处理。
- **504**（**网关超时**）：网关/代理服务器在约定时间内未收到上游服务的完整响应，原因常为上游处理过慢（慢SQL、长逻辑、死锁）、网络延迟高、或网关超时阈值设置过短。排查需后端分析上游服务耗时链路（日志、APM），运维检查网络和超时配置，必要时调大超时时间或优化上游性能。
...
[JSON详细笔记](../../../基础前端/JavaScript/JSON.md)
...
| 原则 | 说明 |
| ------ | ------ |
| **无状态（Stateless）** | 每个请求必须包含**所有必要信息**，服务器不保存客户端上下文 |
| **统一接口（Uniform Interface）** | 使用**标准的 HTTP 方法和格式**进行资源操作 |
| **资源导向（Resource-Based）** | **一切皆为资源**，通过 URI 唯一标识 |
| **可缓存（Cacheable）** | 响应应**明确标示**是否可被缓存 |
| **分层系统（Layered System）** | 客户端无需知道是否直接连接到服务器或中间层 |
...
- **版本控制**：`/v1/users`或`Accept: application/vnd.api.v1+json`
- **使用复数名词**：`/orders`而非`/order`
- **过滤/排序/分页**：`?status=paid&sort=-createdAt&page=2&limit=20`
- **HATEOAS**（可选）：响应中包含相关资源链接，提升可发现性
- **错误统一格式**：
